...
export default function NGOManagementSystem() {
  ...
  const [driveStatus, setDriveStatus] = useState<{ configured: boolean; message: string } | null>(null);
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.error) {
        ...
        setUser(data.user);
        toast({ title: 'Welcome!', description: `Your account is ready, ${data.user.name}!` });
      }
    } catch (error) {
  ...
  const handleGoogleLogin = () => {
    ...
    if (!clientId) {
      ...
      fetch('/api/auth/google/callback?code=demo', { method: 'GET' })
        .then(() => checkSession())
        .then(() => {
          toast({ title: 'Welcome!', description: 'Logged in with Google (Demo Mode)' });
        });
    } else {
      // Real Google OAuth
  ...
  const handleNewRequisition = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/requisitions/new', {
        ...
        body: JSON.stringify({
          ...newReqForm,
          expenseItems: newReqForm.expenseItems.filter(i => i.item && i.amount),
          totalAmount: newReqForm.expenseItems.reduce((sum, i) => sum + (parseFloat(i.amount) || 0), 0),
        }),
      });
      const data = await res.json();
      if (data.error) {
        if (data.pendingRequisitions) {
          toast({ 
            title: 'Action Needed', 
            description: 'Please upload receipts for your past requests before making a new one.',
            variant: 'destructive'
          });
        } else {
          toast({ title: 'Oops!', description: data.error, variant: 'destructive' });
        ...
        toast({ title: 'Success!', description: 'Your request has been sent for review.' });
  ...
  const handleDelete = async () => {
    ...
    try {
      const res = await fetch(`/api/requisitions/update?id=${selectedRequisition.id}`, {
        method: 'DELETE',
      });
      const data = await res.json();
      if (data.error) {
        ...
        toast({ title: 'Deleted!', description: 'The request has been removed.' });
    ...
    try {
      const res = await fetch('/api/requisitions/update', {
        ...
        body: JSON.stringify({
          ...
          totalAmount: editForm.expenseItems.reduce((sum, i) => sum + (parseFloat(i.amount) || 0), 0),
        }),
      });
      const data = await res.json();
      if (data.error) {
        ...
        toast({ title: 'Updated!', description: 'The request has been changed.' });
  ...
  const handleApprove = async (requisitionId: string, action: string) => {
    try {
      ...
      if (data.error) {
        ...
        toast({ title: 'Done!', description: `Request has been ${action.toLowerCase()}ed.` });
        fetchRequisitions();
      }
    } catch (error) {
  ...
  const handleUploadReceipt = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: 'Error', description: 'Could not upload receipt. Please try again.', variant: 'destructive' });
    }
  };
  ...
  const handleExportToSheets = async () => {
    try {
      ...
      toast({ title: 'Error', description: 'Could not export. Please try again.', variant: 'destructive' });
    }
  };
  ...
  const openEditForm = (req: Requisition) => {
    ...
    setEditForm({
      ...
      expenseItems: expenseItems.length > 0 ? expenseItems : [{ item: '', amount: '' }],
      status: req.status,
    });
  ...
  const updateEditExpenseItem = (index: number, field: 'item' | 'amount', value: string) => {
    setEditForm(prev => ({
      ...prev,
      expenseItems: prev.expenseItems.map((item, i) => 
        i === index ? { ...item, [field]: value } : item
  ...
  const calculateTotal = () => {
    return newReqForm.expenseItems.reduce((sum, item) => sum + (parseFloat(item.amount) || 0), 0);
  };

  const calculateEditTotal = () => {
    return editForm.expenseItems.reduce((sum, item) => sum + (parseFloat(item.amount) || 0), 0);
  };

  // Check if user needs to submit receipt (event date has passed)
  const needsToSubmitReceipt = (req: Requisition) => {
    return req.status === 'DISBURSED' && !req.receiptSubmitted && new Date(req.eventDate) < new Date();
  };

  // Check if user can create new requisition
  const canCreateNewRequisition = () => {
    const blockingReqs = requisitions.filter(r => 
      r.userId === user?.id && needsToSubmitReceipt(r)
    );
  ...
  const getBlockingRequisitions = () => {
    return requisitions.filter(r => r.userId === user?.id && needsToSubmitReceipt(r));
  };
  ...
  const pendingForDisbursement = requisitions.filter(r => r.status === 'APPROVED');
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <header className="bg-white border-b shadow-sm sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Building2 className="h-8 w-8 text-slate-700" />
            <div>
              <h1 className="text-xl font-bold text-slate-800">Youth For Christ</h1>
          ...
          <div className="flex items-center gap-4">
            ...
            {driveStatus && (
              <Button 
                ...
                className={driveStatus.configured ? 'text-green-600' : 'text-slate-400'}
              >
        ...
        {needsReceipts.length > 0 && (
          <Alert className="mb-6 border-orange-300 bg-orange-50">
            ...
            <AlertDescription className="text-orange-800">
              <strong>Action Required:</strong> You have {needsReceipts.length} request(s) that need receipts. 
              Please upload receipts for past requests before making new ones.
            </AlertDescription>
          </Alert>
        )}
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold">{pendingForReview.length}</p>
                    </div>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-3xl font-bold">{pendingForDisbursement.length}</p>
                    </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold">{pendingForApproval.length}</p>
                  </div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <TabsList>
              ...
              {['ACCOUNTANT', 'DIRECTOR', 'ADMIN'].includes(user.role) && (
                <TabsTrigger value="approvals">
                  Work Queue
                  {(pendingForReview.length + pendingForApproval.length + pendingForDisbursement.length) > 0 && (
                    <Badge className="ml-2 bg-red-500 text-white">
                      {pendingForReview.length + pendingForApproval.length + pendingForDisbursement.length}
                    </Badge>
                  )}
                </TabsTrigger>
              )}
            ...
            <div className="flex gap-2">
              ...
              <Button onClick={() => setShowNewRequisition(true)} disabled={!canCreateNew}>
              ...
              <Card>
                ...
                <CardContent>
                  <ScrollArea className="h-80">
                    {needsReceipts.length > 0 && (
                      <div className="mb-4">
                        <h4 className="font-medium text-sm text-slate-600 mb-2">Receipts Needed</h4>
                        {needsReceipts.map((req) => (
                          <div key={req.id} className="flex items-center justify-between py-2 px-3 bg-orange-50 rounded-lg mb-2">
                            <div>
                              <p className="font-medium text-sm">{req.reason}</p>
                              <p className="text-xs text-slate-500">
                                Event was on {new Date(req.eventDate).toLocaleDateString()}
                              </p>
                            </div>
                            <Button 
                              size="sm" 
                ...
                <div className="space-y-4">
                          ...
                          <div className="flex-1">
                            <div className="flex flex-wrap items-center gap-2 mb-2">
                              ...
                              {needsToSubmitReceipt(req) && (
                                <Badge variant="destructive">Receipt Required</Badge>
                              )}
                            </div>
                            <h3 className="font-semibold">{req.reason}</h3>
                            <p className="text-sm text-slate-600 mb-2">{req.description}</p>
                            <p className="text-xs text-slate-500">{statusConfig[req.status]?.description}</p>
                  ...
                  {myRequisitions.length === 0 && (
                    <div className="text-center py-12 text-slate-500">
                      ...
                      <p className="text-sm mt-2">Click "New Request" to get started!</p>
                    </div>
                  )}
                </div>
              </CardContent>
          ...
          {['ACCOUNTANT', 'DIRECTOR', 'ADMIN'].includes(user.role) && (
            <TabsContent value="approvals" className="space-y-6">
              ...
              {['ACCOUNTANT', 'ADMIN'].includes(user.role) && pendingForReview.length > 0 && (
                <Card>
                  <CardHeader>
                    <CardTitle>To Check ({pendingForReview.length})</CardTitle>
                    <CardDescription>Verify these requests from staff</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                            ...
                            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                              <div className="flex-1">
                                <div className="flex items-center gap-2 mb-2">
                                  ...
                                  {req.user.department && (
                                    <Badge variant="outline">{req.user.department}</Badge>
                                  )}
                                </div>
                                <h3 className="font-semibold">{req.reason}</h3>
                                <p className="text-sm text-slate-600 mb-2">{req.description}</p>
                                <div className="flex flex-wrap gap-4 text-sm text-slate-500">
                                  <span className="font-semibold">KES {req.totalAmount.toLocaleString()}</span>
                                  <span>Event: {new Date(req.eventDate).toLocaleDateString()}</span>
                                  <span>Needed by: {new Date(req.dateNeeded).toLocaleDateString()}</span>
                                </div>
                              </div>
                              <div className="flex flex-wrap gap-2">
                                <Button
                                ...
                                <Button
                                  size="sm"
                                  onClick={() => handleApprove(req.id, 'CHECK')}
                                >
              ...
              {['DIRECTOR', 'ADMIN'].includes(user.role) && pendingForApproval.length > 0 && (
                <Card>
                  <CardHeader>
                    <CardTitle>To Approve ({pendingForApproval.length})</CardTitle>
                    <CardDescription>These have been checked by the accounts officer</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
              ...
              {['ACCOUNTANT', 'ADMIN'].includes(user.role) && pendingForDisbursement.length > 0 && (
                <Card>
                  <CardHeader>
                    <CardTitle>To Send Money ({pendingForDisbursement.length})</CardTitle>
                    <CardDescription>These requests are approved - send money and mark as disbursed</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                            ...
                            <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                              <div className="flex-1">
                                ...
                                <div className="flex flex-wrap gap-4 text-sm">
                                  ...
                                  <span className="text-slate-500">Event: {new Date(req.eventDate).toLocaleDateString()}</span>
                                </div>
                              </div>
                              <div className="flex flex-wrap gap-2">
                                <Button
                                ...
                                <Button
                                  ...
                                  onClick={() => handleApprove(req.id, 'DISBURSE')}
                                >
                                  <CheckCircle className="h-4 w-4 mr-1" />
                                  Mark as Sent
                                </Button>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <ol className="list-decimal list-inside text-sm text-slate-600 space-y-1">
                ...
                <li>After Director approves, send money and click "Mark as Sent"</li>
            ...
            <div className="bg-amber-50 p-3 rounded-lg">
              <p className="text-sm text-amber-800">
                <strong>Important:</strong> Staff cannot create new requests until all pending receipts are uploaded after the event date.
              </p>
            </div>
            <div className="bg-green-50 p-3 rounded-lg">
              <p className="text-sm text-green-800">
                <strong>Partial Receipts:</strong> If a receipt is for less than the total amount, the balance remains until full amount is covered.
              </p>
            </div>
          </div>
        </DialogContent>
            ...
            {driveStatus?.configured ? (
              <div className="bg-green-50 p-4 rounded-lg">
                ...
                <p className="text-xs text-green-500 mt-2">
                  Your receipts will be automatically backed up to Google Drive when uploaded.
                </p>
              </div>
            ) : (
              <div className="bg-slate-50 p-4 rounded-lg">
                ...
                <p className="text-sm text-slate-500 mb-3">{driveStatus?.message}</p>
          ...
          <form onSubmit={handleNewRequisition} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Select value={newReqForm.reason} onValueChange={(v) => setNewReqForm(prev => ({ ...prev, reason: v }))}>
                  <SelectTrigger>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setNewReqForm(prev => ({ ...prev, accountToCharge: e.target.value }))}
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setNewReqForm(prev => ({ ...prev, eventDate: e.target.value }))}
                  required
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setNewReqForm(prev => ({ ...prev, dateNeeded: e.target.value }))}
                  required
                />
              </div>
            </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setNewReqForm(prev => ({ ...prev, participants: e.target.value }))}
              />
            </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewReqForm(prev => ({ ...prev, transportDistance: e.target.value }))}
                  />
                </div>
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setNewReqForm(prev => ({ ...prev, transportQuantity: e.target.value }))}
                  />
                </div>
              </div>
            )}
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end">
                <div className="bg-slate-100 px-4 py-2 rounded-lg">
                  ...
                  <span className="font-bold text-lg">KES {calculateTotal().toLocaleString()}</span>
                </div>
              </div>
            </div>

            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setShowNewRequisition(false)}>
                Cancel
              </Button>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <form onSubmit={handleEdit} className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <Label>Reason</Label>
                <Select value={editForm.reason} onValueChange={(v) => setEditForm(prev => ({ ...prev, reason: v }))}>
                  <SelectTrigger>
                    <SelectValue placeholder="Select a reason" />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => setEditForm(prev => ({ ...prev, dateNeeded: e.target.value }))}
                />
              </div>
            </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <Label>Item List</Label>
                <Button type="button" variant="outline" size="sm" onClick={addEditExpenseItem}>
                  <Plus className="h-4 w-4 mr-1" />
                  Add Item
                </Button>
              </div>
                ...
                <div key={index} className="flex gap-2 items-start">
                  <div className="flex-1">
                    <Input
                      placeholder="Item name"
                      value={item.item}
                      onChange={(e) => updateEditExpenseItem(index, 'item', e.target.value)}
                    />
                  </div>
                  <div className="w-32">
                    <Input
                      ...
                      placeholder="Amount"
                      value={item.amount}
                      onChange={(e) => updateEditExpenseItem(index, 'amount', e.target.value)}
                    />
                  </div>
              ...
              <div className="flex justify-end">
                <div className="bg-slate-100 px-4 py-2 rounded-lg">
                  <span className="text-sm text-slate-600">Total: </span>
                  <span className="font-bold text-lg">KES {calculateEditTotal().toLocaleString()}</span>
                </div>
              </div>
            </div>

            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setShowEditRequisition(false)}>
                Cancel
              </Button>
          ...
          <form onSubmit={handleUploadReceipt} className="space-y-4">
            <div>
              ...
              <p className="text-xs text-slate-500 mt-1">
                Remaining balance: KES {selectedRequisition ? 
                  ((selectedRequisition.totalAmount - (selectedRequisition.totalReceived || 0))).toLocaleString() : '0'}
              </p>
            </div>
            <div>
              ...
              <Input
                ...
                required
              />
              <p className="text-xs text-slate-500 mt-1">You can upload images or PDF files</p>
            </div>
            <div>
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={() => setShowReceiptUpload(false)}>
                Cancel
              </Button>
      ...
      <Dialog open={showRequisitionDetail} onOpenChange={setShowRequisitionDetail}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
          ...
          {selectedRequisition && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <div>
                  <Badge className={statusConfig[selectedRequisition.status]?.color}>
                ...
                <Button variant="outline" size="sm" onClick={() => handleExport(selectedRequisition.id)}>
                  <Download className="h-4 w-4 mr-1" />
                  Download Form
                </Button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p className="font-medium">{selectedRequisition.user.department || 'Not specified'}</p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-medium text-green-600">KES {selectedRequisition.totalAmount.toLocaleString()}</p>
                </div>
                <div>
                  <Label className="text-sm text-slate-500">Event Date</Label>
                  <p className="font-medium">{new Date(selectedRequisition.eventDate).toLocaleDateString()}</p>
                </div>
                <div>
                  <Label className="text-sm text-slate-500">Date Needed</Label>
                  <p className="font-medium">{new Date(selectedRequisition.dateNeeded).toLocaleDateString()}</p>
                </div>
              </div>
                ...
                <div className="mt-2 border rounded-lg overflow-hidden">
                  <table className="w-full">
                    ...
                    <tbody>
                      {(() => {
                        try {
                          return JSON.parse(selectedRequisition.expenseItems).map((item: ExpenseItem, i: number) => (
                      ...
                      <tr className="border-t bg-slate-50">
                        ...
                        <td className="p-2 text-right font-semibold">{selectedRequisition.totalAmount.toLocaleString()}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              ...
              <div>
                <Label className="text-sm text-slate-500">Amount in Words</Label>
                <p className="mt-1 italic">{numberToWords(Math.floor(selectedRequisition.totalAmount))} Kenya Shillings</p>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-medium">{selectedRequisition.checkedBy?.name || 'Pending'}</p>
                </div>
                <div>
                  <Label className="text-sm text-slate-500">Approved By</Label>
                  <p className="font-medium">{selectedRequisition.approvedBy?.name || 'Pending'}</p>
                </div>
              </div>

              {selectedRequisition.receipts && selectedRequisition.receipts.length > 0 && (
                <>
                  <Separator />
                  <div>
                    <Label className="text-sm text-slate-500">Receipts Uploaded</Label>
                    <div className="mt-2 space-y-2">
                      {selectedRequisition.receipts.map((receipt) => (
                        <div key={receipt.id} className="flex items-center gap-2 p-2 bg-green-50 rounded-lg">
